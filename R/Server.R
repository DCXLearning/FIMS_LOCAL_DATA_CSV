...
server <- function(input, output, session) {
  ...
  ##############################################################################
  month_choices_full <- month_choices
  quarter_to_months <- list(
    Q1 = c("1","2","3"), Q2 = c("4","5","6"),
  ...
  filtered_data_detail <- reactive({
    ...
    df <- nat_raw %>%
      mutate(
        ...
        # area_nat = natfishcatch_areanat              # "1" Freshwater, "2" Marine
      ) %>%
      filter(!is.na(date), 
             !is.na(year), 
             !is.na(catch), 
  ...
  fishing_products_rice_field <- reactive({
    ...
    df_rice <- df %>%
      filter(
        natfishcatch_areanat== "1",
        natfishcatch_typefishing== "3"
      )
  ...
  aquaculture_summary <- reactive({
    ...
    base_data <- aqu_raw %>%
      dplyr::mutate(
        ...
        quarter     = Quarter,
        week        = week,
        area_num    = as.integer(aqufishcatch_areaaqu),
        catch_val   = as.numeric(aqufishcatch_aqucatch),
        fish_code   = as.character(aqufishcatch_aqufishtype),
        # 👉 map to target Khmer label (others become NA and will be excluded)
        fish_label  = unname(target_fish_map[fish_code]),
        province_kh = trimws(province_kh)
      ) %>%
      dplyr::filter(!is.na(catch_val), catch_val > 0)
    
    # ---- Filters (same behavior as before) ----
    if (!is.null(input$year) && input$year != "All") {
      N <- as.integer(input$year)
      base_data <- dplyr::filter(base_data, year %in% c(N - 1L, N))
    }
    if (!is.null(input$Province_Group) && input$Province_Group != "All") {
      base_data <- dplyr::filter(base_data, as.character(Province_Group) == as.character(input$Province_Group))
      if (input$Province_Group == "Inland") {
        base_data <- dplyr::filter(
          base_data,
          as.character(area_code_aqu) %in% c("1","01") |
            grepl("fresh|inland", tolower(as.character(area_en)))
        )
      }
    }
    if (!is.null(input$province) && input$province != "All") {
      base_data <- dplyr::filter(base_data, province_kh == input$province)
    }
    if (!is.null(input$area_en) && input$area_en != "All") {
      base_data <- dplyr::filter(base_data, as.character(area_en) == as.character(input$area_en))
    }
    if (!is.null(input$quarter) && input$quarter != "All") {
      base_data <- dplyr::filter(base_data, quarter == input$quarter)
    }
    if (!is.null(input$month) && input$month != "All") {
      base_data <- dplyr::filter(base_data, month == as.integer(input$month))
    }
    if (!is.null(input$week) && input$week != "All") {
      base_data <- dplyr::filter(base_data, week == input$week)
    }
    
    # ---- Always keep Inland/Marine totals first ----
    special_summary <- base_data %>%
      dplyr::group_by(province_kh, year) %>%
      dplyr::summarise(
        `ត្រីទឹកសាប` = sum(dplyr::if_else(area_num == 1L, catch_val, 0), na.rm = TRUE),
        `ត្រីសមុទ្រ` = sum(dplyr::if_else(area_num == 2L, catch_val, 0), na.rm = TRUE),
        .groups = "drop"
      )
    
    # ---- Target fish only (using Khmer names from the map) ----
    fish_summary <- base_data %>%
      dplyr::filter(!is.na(fish_label)) %>%                        # drop non-target fish
      dplyr::group_by(province_kh, year, fish_label) %>%
      dplyr::summarise(catch = sum(catch_val, na.rm = TRUE), .groups = "drop") %>%
      tidyr::pivot_wider(
        names_from  = fish_label,
        values_from = catch,
        values_fill = 0
      )
    
    # ---- Merge & ensure all expected columns exist ----
    summary_table <- special_summary %>%
      dplyr::left_join(fish_summary, by = c("province_kh", "year")) %>%
      dplyr::mutate(dplyr::across(-c(province_kh, year), ~ tidyr::replace_na(.x, 0)))
    ...
    # ---- Sum by province, order columns, keep Khmer headers ----
    final_data <- summary_table %>%
      dplyr::group_by(province_kh) %>%
      dplyr::summarise(dplyr::across(dplyr::all_of(expected_cols), sum, na.rm = TRUE), .groups = "drop") %>%
      dplyr::mutate(`សរុប` = rowSums(dplyr::across(dplyr::all_of(expected_cols)), na.rm = TRUE)) %>%
      dplyr::rename(`ខេត្ត` = province_kh) %>%
      dplyr::select(`ខេត្ត`, dplyr::all_of(expected_cols), `សរុប`) %>%
      dplyr::arrange(dplyr::desc(`សរុប`)) %>%
      as.data.frame(stringsAsFactors = FALSE, check.names = FALSE)  # <- keep Khmer names
  ...
  aquaculture_summary_detail <- reactive({
    ...
    base_data <- aqu_raw %>%
      dplyr::mutate(
        ...
        catch_val   = as.numeric(aqufishcatch_aqucatch),
        fish_code   = as.character(aqufishcatch_aqufishtype),
        # ✅ use Khmer fish name if available; fall back to code
        fish_label  = dplyr::coalesce(fish_name_kh, fish_code),
        province_kh = trimws(province_kh)
      ) %>%
      dplyr::filter(!is.na(catch_val), catch_val > 0)
    ...
    # ---- Inland vs Marine totals (always keep these 2 first) ----
    special_summary <- base_data %>%
      dplyr::group_by(province_kh, year) %>%
      dplyr::summarise(
        `ត្រីទឹកសាប` = sum(dplyr::if_else(area_num == 1L, catch_val, 0), na.rm = TRUE),
    ...
    # ---- All fish types (Khmer labels, not codes) ----
    fish_summary <- base_data %>%
      dplyr::filter(!is.na(fish_label)) %>%
      dplyr::group_by(province_kh, year, fish_label) %>%
      dplyr::summarise(catch = sum(catch_val, na.rm = TRUE), .groups = "drop") %>%
      tidyr::pivot_wider(
        names_from  = fish_label,
    ...
    # ---- Merge and total by province ----
    summary_table <- special_summary %>%
      dplyr::left_join(fish_summary, by = c("province_kh", "year")) %>%
      dplyr::mutate(dplyr::across(-c(province_kh, year), ~ tidyr::replace_na(.x, 0)))
    
    fish_cols <- setdiff(colnames(summary_table), c("province_kh", "year"))
    
    final_data <- summary_table %>%
      dplyr::group_by(province_kh) %>%
      dplyr::summarise(dplyr::across(dplyr::all_of(fish_cols), sum, na.rm = TRUE), .groups = "drop") %>%
      dplyr::mutate(`សរុប` = rowSums(dplyr::across(dplyr::all_of(fish_cols)), na.rm = TRUE)) %>%
      dplyr::rename(`ខេត្ត` = province_kh) %>%
      # keep non-syntactic (Khmer) column names as-is
...
#################################################################################
  ...
  # When QUARTER changes, limit the MONTH options to only that quarter
  observeEvent(input$quarter, ignoreInit = TRUE, {
    if (is.null(input$quarter) || input$quarter == "All") {
      # restore full month list
      sel <- if (!is.null(input$month) && input$month %in% unname(month_choices_full)) input$month else "All"
      updateSelectInput(
        session, "month",
        choices  = month_choices_full,
        selected = sel
      )
    } else {
      qm <- quarter_to_months[[input$quarter]]
      # keep only months that exist in your data (values of month_choices_full)
      allowed_vals <- intersect(qm, unname(month_choices_full))
      filtered_month_choices <- c("All" = "All", month_choices_full[unname(month_choices_full) %in% allowed_vals])
      
      sel <- input$month
      if (is.null(sel) || !(sel %in% allowed_vals)) sel <- "All"
      
      updateSelectInput(
        session, "month",
        choices  = filtered_month_choices,
        selected = sel
      )
    }
  })
  
  # OPTIONAL: when MONTH changes, align QUARTER automatically (keeps them consistent)
  observeEvent(input$month, ignoreInit = TRUE, {
    mo <- suppressWarnings(as.integer(input$month))
    if (!is.na(mo)) {
      q_for_mo <- paste0("Q", ((mo - 1L) %/% 3L) + 1L)
      if (!is.null(input$quarter) && input$quarter != "All" && input$quarter != q_for_mo) {
        updateSelectInput(session, "quarter", selected = q_for_mo)
      }
    }
  })
  ...
  observeEvent(input$syncBtn, {
    ...
    withProgress(message = "Syncing data…", value = 0, {
      tryCatch({
        incProgress(0.3, detail = "Connecting & exporting")
        refreshData()
        incProgress(1, detail = "Done")
        showNotification("✅ Data synchronized and CSV files updated!", type = "message", duration = 4)
      }, error = function(e) {
