...
nat_raw <- nat_df %>%
  ...
  mutate(
    ...
    week                      = paste0("Week", ceiling(day(date) / 7)),
    catch                     = as.numeric(natfishcatch_natcatch),
    area_code_nat             = as.character(natfishcatch_areanat),
    natfishcatch_typefishing  = as.character(natfishcatch_typefishing),
    natfishcatch_nat_fishtype = as.character(natfishcatch_natfishtype),
  ...
  left_join(province_lookup,      by = c("province" = "province_code")) %>%
  left_join(fish_label_lookup,    by = c("natfishcatch_natfishtype" = "fish_code")) %>%
  left_join(area_lookup,          by = c("area_code_nat" = "area_code")) %>%
  mutate(
    province_kh  = coalesce(province_kh, province),
    province_en  = coalesce(province_en, province),
    fish_name_kh = coalesce(fish_name_kh, natfishcatch_natfishtype),
    fish_name_en = coalesce(fish_name_en, natfishcatch_natfishtype),
...
aqu_raw <- aqu_df %>%
  ...
  mutate(
    ...
    week         = paste0("Week", ceiling(day(date) / 7)),
    catch        = as.numeric(aqufishcatch_aqucatch),
    area_code_aqu= as.character(aqufishcatch_areaaqu),
    aqu_type     = as.character(aqufishcatch_aqutype),
    aqu_fishtype = as.character(aqufishcatch_aqufishtype)
  ) %>%
  left_join(province_lookup, by = c("province" = "province_code")) %>%
  left_join(fish_label_lookup, by = c("aqufishcatch_aqufishtype" = "fish_code")) %>%
...
processing_raw <- processing_df %>%
  ...
  mutate(
    ...
    area_code_pro            = as.character(processing_areapro)
  ) %>%
  left_join(province_lookup,           by = c("province" = "province_code")) %>%
...
all_fish_data_raw <- bind_rows(
  nat_raw %>%
    select(Year, Month, Quarter, catch, province_en,province_kh, area_en, area_code_nat,Province_Group,Month) %>%
  ...
  aqu_raw %>%
    select(Year, Month, Quarter, catch, province_en,province_kh,Province_Group,Month) %>%
    rename(province = province_en) %>%
...
# Years
all_years <- sort(unique(c(nat_raw$Year, aqu_raw$Year, processing_raw$Year, patrol_raw$Year, all_fish_data_raw$Year)))
...
# Quarters
all_quarter <- sort(unique(c(nat_raw$Quarter, aqu_raw$Quarter, processing_raw$Quarter, patrol_raw$Quarter,all_fish_data_raw$Quarter)))
...
# Months
all_month <- sort(unique(c(nat_raw$Month, aqu_raw$Month, processing_raw$Month, patrol_raw$Month, all_fish_data_raw$Month)))
all_month <- all_month[!is.na(all_month)]

# Build choices for the Province_Group selector
groups <- unique(as.character(nat_raw$Province_Group, aqu_raw$Province_Group,processing_raw$Province_Group,patrol_raw$Province_Group, all_fish_data_raw$Province_Group))
...
all_provinces <- nat_raw %>%
  dplyr::bind_rows(aqu_raw, processing_raw, patrol_raw,all_fish_data_raw) %>%
...
all_areas <- sort(unique(c(nat_raw$area_en, processing_raw$area_en, aqu_raw$area_en, all_fish_data_raw$area_en)))
